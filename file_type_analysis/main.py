...
class analysis_file():
    ...
    def read_extensions(self):
        a = dict()
        maxstring = self.count_lines(self.indexfile)
        with open(self.indexfile,'r') as indefile:
            count_string = 0
            for line in indefile:
                string = line.split(";")
                ext = re.match(r'.*\.(.*)$', string[self.unpack_map['orig']].decode('utf-8'),re.I | re.U)
                if not ext:
                    continue
                ext = ext.group(1).lower()
                path = '/'.join([self.dir] + [string[i].replace('"','') for i in self.unpack_map['path']] + [string[self.unpack_map['filename']].replace('"','')+'.'+self.type_file])
                if os.path.isfile(path):
                    if not ext in a:
                        a[ext] = 0
                    else:
                        a[ext] += os.path.getsize(path)
                self.progress(count_string, maxstring)
                count_string += 1
        return a
    ...
    def progress(self,i,maxfile):
        persent = int( 100/float(maxfile)*i)
        sys.stdout.write('\r')
        # the exact output you're looking for:
        sys.stdout.write("[%-100s] %d%%" % ('=' * persent, persent))
        sys.stdout.flush()
